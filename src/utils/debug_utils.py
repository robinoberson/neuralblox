...
def save_visualization(trainer, dataloader, idx_config, cfg):
    ...
    for batch_idx, (input_tensor, output_tensor, p_stacked, p_n_stacked) in enumerate(dataloader):
        ...
        print(f'Saving visualization for batch {batch_idx}')
...
def train_and_evaluate(trainer, dataloader, cfg, idx_config, num_iter=15000, lr=0.01, save_checkpoint=False, checkpoint_io_merging=None, experiment = None, debug = False):
    ...
    criterion = nn.L1Loss()
    optimizer = optim.Adam(trainer.model_merge.parameters(), lr=lr)
    
    # Learning rate scheduler
    scheduler = optim.lr_scheduler.ReduceLROnPlateau(optimizer, mode='min', patience=100, factor=0.9)
