...
for discard_patch in [False, True]:
    ...
    for is_neuralblox in [False, True]:
        ...
        for n_max_inputs in cfg['evaluation']['n_max_inputs']:
            ...
            files = sorted([f for f in sorted(os.listdir(processed_data_dir)) if (f.startswith(f'data_saving_{n_max_inputs}') and f'og{is_neuralblox}' in f and f'dispatch-{cfg["evaluation"]["discard_patches"]["bool"]}' in f)])
            n_files = len(files)
            print(f'Number of files for n_max_inputs = {n_max_inputs}: {n_files}')
            for file in files:
                batch_path = os.path.join(processed_data_dir, file)
                print(f'Computing metrics for {batch_path}, is neuralblox = {cfg["evaluation"]["is_neuralblox"]}, discard patch = {cfg["evaluation"]["discard_patches"]["bool"]}')
                print(datetime.fromtimestamp(os.path.getctime(batch_path)).strftime('%Y-%m-%d %H:%M:%S'))
            ...
            metrics_is_neuralblox[n_max_inputs] = metrics_n_max_inputs
        metrics_patch[is_neuralblox] = metrics_is_neuralblox

    full_metrics[discard_patch] = metrics_patch

file_path = os.path.join(processed_data_dir, f'full_metrics_with_ground.pth')
